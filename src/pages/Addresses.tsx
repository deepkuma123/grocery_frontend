...
const Addresses = () => {
    ...
    const [form, setForm] = useState({ label: "", address: "", city: "", state: "", zip: "", isDefault: false, lat: 51.505, lng: -0.09 });

    const resetForm = () => {
        setForm({ label: "", address: "", city: "", state: "", zip: "", isDefault: false, lat: 51.505, lng: -0.09 });
        setShowForm(false);
        setEditingId(null);
    };
    ...
    const onEditHandler = (add: Address) => {
        setForm({ label: add.label, address: add.address, city: add.city, state: add.state, zip: add.zip, isDefault: add.isDefault, lat: add.lat || 51.505, lng: add.lng || -0.09 });
        setEditingId(add.id);
        setShowForm(true);
    };
                ...
                <div className="flex items-center justify-between mb-8">
                    ...
                    <button
                        onClick={async () => {
                            resetForm();
                            try {
                                const coords = await getLocation(1);
