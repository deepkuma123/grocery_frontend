        ...
        try {
            const orderData = {
                items: items.map((item) => ({
                    ...
                    price: item.variantId && item.product.variants ? item.product.variants.find((v: any) => v._id === item.variantId)?.price || item.product.price : item.product.price,
                    name: item.variantId && item.product.variants ? `${item.product.name} - ${item.product.variants.find((v: any) => v._id === item.variantId)?.unit}` : item.product.name,
                ...
                <div className="grid md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white rounded-2xl p-5 h-fit sticky top-24">
                        ...
                        <div className="mt-6 pt-6 border-t border-app-border">
                            <h4 className="text-sm font-semibold text-app-green mb-3">Promo Code</h4>
                            <div className="flex gap-2">
                                <input 
                                    ...
                                    className="w-full text-sm p-2.5 border border-app-border rounded-lg focus:outline-none focus:ring-1 focus:ring-app-green"
                                    disabled={!!appliedCoupon}
                                />
                                {appliedCoupon ? (
                                    <button 
                                        onClick={() => { setAppliedCoupon(""); setCouponCode(""); setDiscount(0); }} 
                                        className="px-4 py-2.5 bg-red-100 text-red-600 rounded-lg text-sm font-medium hover:bg-red-200 transition-colors"
                                    >
                                        Remove
                                    </button>
                                ) : (
                                    <button 
                                        ...
                                        className="px-4 py-2.5 bg-app-orange text-white rounded-lg text-sm font-medium hover:bg-app-orange-dark transition-colors"
                                    >
                                        Apply
                                    </button>
                                )}
