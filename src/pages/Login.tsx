...
import { Loader2Icon, LockIcon, MailIcon, UserIcon, PhoneIcon } from "lucide-react";
        ...
        <div className="min-h-screen flex">
            ...
            <div className="hidden lg:flex lg:w-1/2 bg-app-green relative items-center justify-center">
                ...
                <div className="relative text-center px-12">
                    <h2 className="text-4xl font-semibold text-white mb-4">Welcome back to DailySewa</h2>
                ...
                <div className="w-full max-w-md">
                    ...
                    <div className="text-center mb-8">
                        <Link to="/" className="inline-flex items-center gap-2 mb-6">
                            <img src={assets.dailysewa_nav_logo} alt="DailySewa" className="h-10" />
                        </Link>
                    ...
                    <div className="flex bg-white rounded-xl p-1 mb-6 border border-app-border">
                        <button 
                            className={`flex-1 py-2 text-sm font-medium rounded-lg transition-colors ${loginMethod === "email" ? "bg-app-green text-white shadow" : "text-zinc-500 hover:text-zinc-800"}`}
                            onClick={() => { setLoginMethod("email"); setIsLoginState(true); }}
                        >
                            Email
                        </button>
                        <button 
                            className={`flex-1 py-2 text-sm font-medium rounded-lg transition-colors ${loginMethod === "phone" ? "bg-app-green text-white shadow" : "text-zinc-500 hover:text-zinc-800"}`}
                            onClick={() => { setLoginMethod("phone"); setIsLoginState(true); }}
                        >
                            Phone
                        </button>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <>
                                <label className="text-sm flex flex-col gap-1">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} required placeholder="+91 98765 43210" className="w-full pl-11 pr-4 py-3 text-sm bg-white rounded-xl border not-focus:border-app-border transition-all" disabled={otpSent} />
                                    </div>
                                </label>
                                {otpSent && (
                                    <label className="text-sm flex flex-col gap-1">
                                        ...
                                        <div className="relative">
                                            <LockIcon className="absolute left-3.5 top-1/2 -translate-y-1/2 size-4 text-app-text-light" />
                                            <input type="text" value={otp} onChange={(e) => setOtp(e.target.value)} required placeholder="Enter 6-digit OTP" className="w-full pl-11 pr-4 py-3 text-sm bg-white rounded-xl border not-focus:border-app-border transition-all" />
                                        </div>
                                    </label>
                                )}
                                <button type="button" onClick={() => {
                                    if (!otpSent) setOtpSent(true);
                                    else toast.error("OTP login is coming soon!");
                                }} disabled={loading} className="flex-center w-full py-3 bg-green-950 text-white font-semibold rounded-xl hover:bg-green-900 transition-colors disabled:opacity-50">
                                    {loading ? <Loader2Icon className="animate-spin" /> : otpSent ? "Verify OTP & Login" : "Send OTP"}
                                </button>
                            </>
                        )}
                        ...
                        <div className="relative my-6">
                            ...
                            <div className="relative flex justify-center text-sm">
                                <span className="px-2 bg-app-cream text-app-text-light">Or continue with</span>
                            </div>
                        </div>

                        <button type="button" onClick={() => toast.error("Google login coming soon!")} className="flex-center w-full py-3 bg-white border border-app-border text-zinc-700 font-semibold rounded-xl hover:bg-gray-50 transition-colors gap-3">
